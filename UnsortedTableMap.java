...
public class UnsortedTableMap<K, V> extends AbstractMap<K,V> 
{
	...
	/** Associates given value with given key, replacing a previous one(if any) */
	public V put(K key, V value)
	{
		...
		else return table.get(j).setValue(value);	//key already exists, so replaced value is returned
	}
	...
	public V remove(K key)
	{
		...
		if (j != n-1) table.set(j,  table.get(n-1));	// relocate last entry to 'hole' created by removal
